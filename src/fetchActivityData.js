...
const REGEXES = {
  ...
  FREE_MISSION_COMPLETED: /^Free mission completed\s?: (?<battleType>\S+) (?<battleId>\d+) \((?<numMissions>\d+) missions?\)/,
  SUBMITTED_BATTLE_REVIEW: /^Submitted review for (?:battle )?(?<battleType>\S+) (?<battleId>\d+)/,
  SUBMITTED_PATCH_REVIEW: /^Submitted review for (?<patchName>\S+) Patch/,
  ...
  NEW_FCHG: /^New Fleet Commander's Honor Guard rank(?:achieved\s?)?: (.*) \[(.*)\]/,
  ...
  NEW_UNIFORM_APPROVED: /^New uniform (?:upload|image) approved/,
  ...
  UPLOADED_FICTION: /^Submitted a new fiction item: (?<title>.+)/,
  ...
  JOINED_TC: /Joined the Emperor's Hammer TIE Corps as (?:Cadet )?(?<name>.*)!?/,
  JOINED_DB: /Joined the Dark Brotherhood as (?:Cadet )?(?<name>.*)!?/,
  JOINED_SO: /Joined the Secret Order as (?:Cadet )?(?<name>.*)!?/,
